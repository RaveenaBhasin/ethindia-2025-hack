...
const provider = new ethers_1.ethers.JsonRpcProvider(arbRpc);
const signer = new ethers_1.ethers.Wallet(relayerPrivateKey, provider);
...
async function wrapETH(amountInETH) {
    ...
    const tx = await weth.deposit({ value: ethers_1.ethers.parseEther(amountInETH) });
