...
class MultiChainAcrossDataWorker {
    constructor(config) {
        this.chainWorkers = new Map();
        this.relayData = [];
        this.relayerRefunds = new Map();
        this.relayerRefundLeaves = [];
        this.poolRebalanceLeaves = [];
        this.isRunning = false;
        this.refundCounter = 0;
        this.config = config;
        this.hubPoolProvider = new ethers_1.ethers.JsonRpcProvider(config.hubPool.rpc);
        this.hubPoolWallet = new ethers_1.ethers.Wallet(config.hubPool.privateKey, this.hubPoolProvider);
        const hubPoolAbi = [
        ...
        this.hubPool = new ethers_1.ethers.Contract(config.hubPool.address, hubPoolAbi, this.hubPoolWallet);
        ...
        this.minRefundVolume = ethers_1.ethers.parseEther("0");
    }
    initializeChainWorkers() {
        const spokePoolAbi = [
        ...
        for (const chainConfig of this.config.chains) {
            const provider = new ethers_1.ethers.JsonRpcProvider(chainConfig.rpc);
            const wallet = new ethers_1.ethers.Wallet(this.config.hubPool.privateKey, provider);
            const spokePool = new ethers_1.ethers.Contract(chainConfig.spokePoolAddress, spokePoolAbi, wallet);
            const chainWorker = {
    ...
    async initializeLastProcessedBlocks() {
    ...
    async processRelayForRefund(relay) {
        ...
        await this.redis.hSet(`refunds:${relayerAddress}`, `${refund.chainId}:${refund.refundCounter}`, JSON.stringify(refund));
        console.log(`💰 Queued refund for relayer ${relayerAddress}: ${ethers_1.ethers.formatEther(refund.amount)} tokens on chain ${repaymentChainId}`);
    }
    ...
    async buildRelayerRefundRoot() {
        ...
        const refundsByChainAndToken = new Map();
    ...
    async processRelayData() {
        ...
        try {
            ...
            const slowRelayRoot = ethers_1.ethers.ZeroHash;
    ...
    async executeRootBundleOnHubPool() {
        ...
        try {
            ...
            for (const leaf of poolRebalanceLeaves) {
                ...
                try {
                    ...
                    const tx = await this.hubPool.executeRootBundle(leaf.chainId, leaf.groupIndex, leaf.bundleLpFees, leaf.netSendAmounts, leaf.runningBalances, leaf.leafId, leaf.l1Tokens, proof);
                    await tx.wait();
    ...
    async executeRefundsOnAllChains() {
    ...
    async executeRefundsOnSpokePool(spokePool, chainId) {
        ...
        const leaves = this.relayerRefundLeaves.map(leaf => {
            const leafData = ethers_1.ethers.AbiCoder.defaultAbiCoder().encode(['uint256', 'uint256', 'uint256[]', 'uint32', 'address', 'address[]'], [
                leaf.amountToReturn,
                leaf.chainId,
                leaf.refundAmounts,
                leaf.leafId,
            ...
            return ethers_1.ethers.keccak256(leafData);
        });
        ...
        for (const refundLeaf of chainRefunds) {
            try {
                ...
                const leafHash = ethers_1.ethers.keccak256(leafData);
                ...
                const refundAmounts = refundLeaf.refundAmounts.map(amount => typeof amount === 'string' ? BigInt(amount) : amount);
                const tx = await spokePool.executeRelayerRefundLeaf(1, // FIXME: Hardcoding only for testing
                {
                ...
                for (let i = 0; i < refundLeaf.refundAddresses.length; i++) {
                    console.log(`  - Refunded ${ethers_1.ethers.formatEther(refundLeaf.refundAmounts[i])} to ${refundLeaf.refundAddresses[i]}`);
                }
            }
            catch (error) {
    ...
    bytes32ToAddress(bytes32) {
        return ethers_1.ethers.getAddress('0x' + bytes32.slice(-40));
    }
    createPoolRebalanceMerkleTree(leaves) {
        const leafHashes = leaves.map(leaf => this.hashPoolRebalanceLeaf(leaf));
        return new merkletreejs_1.MerkleTree(leafHashes, ethers_1.ethers.keccak256, { sortPairs: true });
    }
    hashPoolRebalanceLeaf(leaf) {
        const cleanedLeaf = {
            ...
            l1Tokens: (leaf.l1Tokens || []).map(addr => ethers_1.ethers.getAddress(addr))
        };
        const encoded = ethers_1.ethers.AbiCoder.defaultAbiCoder().encode([
            "tuple(" +
                "uint256 chainId," +
    ...
    generatePoolRebalanceProofForLeaf(merkleTree, leaf) {
        const leafHash = this.hashPoolRebalanceLeaf(leaf);
        return merkleTree.getHexProof(leafHash);
    }
    hashRelayerRefundLeaf(leaf) {
        const cleanedLeaf = {
            amountToReturn: BigInt(leaf.amountToReturn),
            chainId: BigInt(leaf.chainId),
            refundAmounts: (leaf.refundAmounts || []).map(BigInt),
            leafId: leaf.leafId,
            l2TokenAddress: ethers_1.ethers.getAddress(leaf.l2TokenAddress),
            refundAddresses: (leaf.refundAddresses || []).map(addr => ethers_1.ethers.getAddress(addr))
        };
        const encoded = ethers_1.ethers.AbiCoder.defaultAbiCoder().encode([
            "tuple(" +
    ...
    createRelayerRefundMerkleTree(leaves) {
    ...
    if (chainsConfigStr) {
        try {
            ...
            for (const [chainId, chainData] of Object.entries(chainsData)) {
                chains.push({
                    chainId: parseInt(chainId),
                    type: chainData.type || `chain-${chainId}`,
                    rpc: chainData.rpc,
                    spokePoolAddress: chainData.spokePoolAddress
                });
            }
        }
        catch (error) {
