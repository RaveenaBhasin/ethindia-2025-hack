...
const MAINNET_RPC_URL = process.env.MAINNET_RPC_URL || "https://rpc.ankr.com/eth";
const PRIVATE_KEY = process.env.PRIVATE_KEY;
if (!PRIVATE_KEY) {
    throw new Error('PRIVATE_KEY is not set in environment variables');
}
const provider = new ethers_1.ethers.JsonRpcProvider(MAINNET_RPC_URL);
...
async function swapETHToUSDC(amountInETH, minAmountOutUSDC) {
    try {
        console.log(`Starting swap of ${amountInETH} ETH to USDC...`);
        // Convert ETH amount to wei
        const amountIn = ethers_1.ethers.parseEther(amountInETH);
        // Create contract instances
        const weth = new ethers_1.ethers.Contract(WETH_ADDRESS, WETH_ABI, signer);
        const router = new ethers_1.ethers.Contract(UNISWAP_V3_ROUTER_ADDRESS, UNISWAP_V3_ROUTER_ABI, signer);
        ...
        const minAmountOut = minAmountOutUSDC ? ethers_1.ethers.parseUnits(minAmountOutUSDC, 6) : 0; // USDC has 6 decimals
        const swapParams = {
